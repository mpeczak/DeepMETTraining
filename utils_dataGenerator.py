...
class HDF5DataGenerator(Sequence):
    ...
    def __init__(self, hdf5_file, batch_size, shuffle=True):
        ...
        self.indexes = np.arange(len(self.hdf5_file['X']))
        if self.shuffle:
            np.random.shuffle(self.indexes)

    def __len__(self):
        return int(np.ceil(len(self.hdf5_file['X']) / float(self.batch_size)))

    def __getitem__(self, index):
        indexes = self.indexes[index*self.batch_size:(index+1)*self.batch_size]
        X = np.array([self.hdf5_file['X'][i] for i in indexes])
        Y = np.array([self.hdf5_file['Y'][i] for i in indexes])
        return X, Y
